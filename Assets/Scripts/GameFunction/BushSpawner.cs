using UnityEngine;
using System.Collections.Generic;

public class BushSpawner : MonoBehaviour
{
    [Header("Bush Prefabs")]
    public GameObject[] bushPrefabs;

    [Header("Bush Parent")]
    public GameObject bushesParent;

    [Header("Bush Area")]
    public float startX;
    public float endX;
    public float startY;
    public float endY;

    [Header("Bush Spawn Amount")]
    public int bushAmount = 50;

    [Header("Spawn Protection Areas")]
    public List<SpawnProtection> spawnProtectionAreas = new List<SpawnProtection>();

    [System.Serializable]
    public class SpawnProtection
    {
        public float startX;
        public float endX;
        public float startY;
        public float endY;
    }

    void Start()
    {
        SpawnBushes();
    }

    void SpawnBushes()
    {
        if (bushPrefabs.Length == 0)
        {
            Debug.LogWarning("No bush prefabs have been added!");
            return;
        }

        for (int i = 0; i < bushAmount; i++)
        {
            Vector2 spawnPosition = GetRandomSpawnPosition();

            // Pick a random bush prefab
            GameObject randomBush =
                bushPrefabs[Random.Range(0, bushPrefabs.Length)];

            // Spawn the bush
            GameObject bush = Instantiate(
                randomBush,
                spawnPosition,
                Quaternion.identity,
                bushesParent.transform
            );
        }
    }

    Vector2 GetRandomSpawnPosition()
    {
        Vector2 position;

        // Keep trying until we find a valid position
        do
        {
            float randomX = Random.Range(startX, endX);
            float randomY = Random.Range(startY, endY);

            position = new Vector2(randomX, randomY);

        } while (IsProtected(position));

        return position;
    }

    bool IsProtected(Vector2 position)
    {
        foreach (SpawnProtection area in spawnProtectionAreas)
        {
            if (position.x >= area.startX &&
                position.x <= area.endX &&
                position.y >= area.startY &&
                position.y <= area.endY)
            {
                return true;
            }
        }

        return false;
    }
}